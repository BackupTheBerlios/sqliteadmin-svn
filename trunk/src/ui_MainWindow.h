...
public:
    ...
    tablesList = new TableListWidget(layoutWidget);
    tablesList->setObjectName(QString::fromUtf8("tablesList"));
    tablesList->setContextMenuPolicy(Qt::DefaultContextMenu);
    ...
    QObject::connect(clearSqlButton, SIGNAL(clicked()), sqlTextEdit, SLOT(clear()));
    QObject::connect(saveSqlButton, SIGNAL(clicked()), actionSa_ve_SQL_File, SLOT(trigger()));
    QObject::connect(clearHistoryButton, SIGNAL(clicked()), historyList, SLOT(clear()));
    ...
    actionC_lose->setText(QApplication::translate("MainWindow", "C&lose Database File", 0, QApplication::UnicodeUTF8));
